...
package mjDataAccess.mjHelpers;
...
import mjInfrastructure.AppConfig;
import mjInfrastructure.AppException;

public class DataHelperSQLiteDAO <T> implements ImjDAO<T> {
    ...
    public boolean create(T entity) throws AppException {
        ...
        for (Field field : fields) {
            ...
            if (!name.equalsIgnoreCase(tablePK)
                && !name.equalsIgnoreCase("mjEstado")
                && !name.equalsIgnoreCase("mjFechaCreacion")
                && !name.equalsIgnoreCase("mjFechaModifica")) {
    ...
    public boolean update(T entity) throws AppException {
        try {
            ...
            updates.append("mjFechaModifica = ?"); // campo técnico de auditoría
    ...
    public boolean delete(Integer id) throws AppException {
        String sql = String.format("UPDATE %s SET mjEstado = ?, mjFechaModifica = ? WHERE %s = ?", tableName, tablePK);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            stmt.setString(1, "X");
    ...
    public T readBy(Integer id) throws AppException {
        String sql = String.format("SELECT * FROM %s WHERE %s = ? AND mjEstado = 'A'", tableName, tablePK);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            stmt.setInt(1, id);
    ...
    public List<T> readAll() throws AppException {
        ...
        String sql = String.format("SELECT * FROM %s WHERE mjEstado = 'A'", tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
    ...
    public Integer getMaxReg() throws AppException {
        String sql = String.format("SELECT COUNT(*) FROM %s WHERE mjEstado = 'A'", tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
